        ...
        implements ConnectorMetadata, TransactionalMetadata
{
        ...
        return Optional.of(new SystemTable()
        {
            ...
            public RecordCursor cursor(ConnectorTransactionHandle transactionHandle, ConnectorSession session, TupleDomain<Integer> constraint)
            {
                ...
                try {
                    ...
                    org.apache.iceberg.Table icebergTable = icebergUtil.getIcebergTable(schemaName, sourceTableName.getTableName(), getConfiguration(session, schemaName));
            ...
            Collection<ComputedStatistics> computedStatistics)
    {
        ...
        org.apache.iceberg.types.Type[] partitionColumnTypes = icebergTable.getInputColumns().stream()
                ...
                .map(type -> TypeConverter.convert(type))
    ...
    public void addColumn(ConnectorSession session, ConnectorTableHandle tableHandle, ColumnMetadata column)
    {
        ...
        icebergTable.updateSchema().addColumn(column.getName(), TypeConverter.convert(column.getType())).commit();
    }
    ...
    private List<ColumnMetadata> getColumnMetadatas(org.apache.iceberg.Table icebergTable)
    {
        List<ColumnMetadata> columnMetadatas = icebergTable.schema().columns().stream()
                .map(c -> new ColumnMetadata(c.name(), TypeConverter.convert(c.type(), typeManager)))
                .collect(toList());
    ...
    private List<Types.NestedField> toIceberg(List<ColumnMetadata> columns)
    {
        ...
        for (ColumnMetadata column : columns) {
            if (!column.isHidden()) {
                ...
                org.apache.iceberg.types.Type icebergType = TypeConverter.convert(type);
